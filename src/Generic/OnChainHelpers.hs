...
This module provides a set of functions for performing scaled arithmetic operations
with precise rounding control. These functions are particularly useful for financial
calculations or token manipulations where maintaining exact precision is crucial.

Scaling Mechanism:
- Numbers are scaled up by a factor (e.g., 1e6, 1e2, or 10e6 for basis points)
  to allow for fractional values while using integer arithmetic.
- For example, 1.23 would be represented as 1230000 when using 1e6 scaling.

Scaling factors:
- 1e6: For general purpose scaling (e.g., 1.23 -> 1230000)
- BPx1e3: For basis points calculations (1 BP = 0.01%, 10000 BP = 100%)
- 1e2: For percentage calculations (e.g., 1.23% -> 123)

Rounding Mechanisms:
1. Round Up: Rounds away from zero for both positive and negative numbers.
   Used in `multiply_By_Scaled_And_RoundUp` and its derivatives.

2. Round Down: Rounds towards zero for both positive and negative numbers.
   Used in `multiply_By_Scaled_And_RoundDown` and its derivatives.

3. Safe Round Down: Used in division operations, rounding towards zero.
   Implemented in `divide_By_Scaled_And_RoundDownSafe` and its derivatives.

Important Note:
While these functions can handle negative numbers, they are primarily intended
for use with positive numbers in normal circumstances. The behavior with negative
numbers is designed to mimic PlutusTx.Ratio.truncate for consistency.

Function Families:
1. Multiplication Functions:
   - Generic: multiply_By_Scaled_And_RoundUp, multiply_By_Scaled_And_RoundDown
   - 1e6 scale: multiply_By_Scaled_1e6_And_RoundUp, multiply_By_Scaled_1e6_And_RoundDown
   - BPx1e3 scale: multiply_By_Scaled_BPx1e3_And_RoundUp, multiply_By_Scaled_BPx1e3_And_RoundDown
   - 1e2 scale: multiply_By_Scaled_1e2_And_RoundUp, multiply_By_Scaled_1e2_And_RoundDown

2. Division Functions:
   - Generic: divide_By_Scaled_And_RoundDownSafe
   - 1e6 scale: divide_By_Scaled_1e6_And_RoundDownSafe
   - BPx1e3 scale: divide_By_Scaled_BPx1e3_And_RoundDownSafe
   - 1e2 scale: divide_By_Scaled_1e2_And_RoundDownSafe

Truncation Behavior:
- For positive numbers:
  - RoundUp: Rounds away from zero (ceiling)
  - RoundDown: Rounds towards zero (floor)
- For negative numbers:
  - RoundUp: Rounds away from zero (floor)
  - RoundDown: Rounds towards zero (ceiling)

This behavior ensures consistency with PlutusTx.Ratio.truncate, which always
rounds towards zero for both positive and negative numbers.

Examples (using 1e6 scaling):

1. Multiplication (RoundUp):
   multiply_By_Scaled_1e6_And_RoundUp 100 1050000 = 105
   multiply_By_Scaled_1e6_And_RoundUp 100 1049999 = 105
   multiply_By_Scaled_1e6_And_RoundUp (-100) 1050000 = -105
   multiply_By_Scaled_1e6_And_RoundUp (-100) 1049999 = -105

2. Multiplication (RoundDown):
   multiply_By_Scaled_1e6_And_RoundDown 100 1050000 = 105
   multiply_By_Scaled_1e6_And_RoundDown 100 1049999 = 104
   multiply_By_Scaled_1e6_And_RoundDown (-100) 1050000 = -105
   multiply_By_Scaled_1e6_And_RoundDown (-100) 1049999 = -104

3. Division (Safe RoundDown):
   divide_By_Scaled_1e6_And_RoundDownSafe 105 1000000 = 105
   divide_By_Scaled_1e6_And_RoundDownSafe 105 1050000 = 100
   divide_By_Scaled_1e6_And_RoundDownSafe (-105) 1000000 = -105
   divide_By_Scaled_1e6_And_RoundDownSafe (-105) 1050000 = -100
   divide_By_Scaled_1e6_And_RoundDownSafe 105 (-1000000) = -105
   divide_By_Scaled_1e6_And_RoundDownSafe (-105) (-1000000) = 105
...
4. Edge cases:
   multiply_By_Scaled_1e6_And_RoundUp 1000000 1 = 1
   divide_By_Scaled_1e6_And_RoundDownSafe 1 1 = 1000000
   divide_By_Scaled_1e6_And_RoundDownSafe 1 (maxBound :: Int) = 0
   multiply_By_Scaled_1e6_And_RoundUp 0 1234567 = 0
   divide_By_Scaled_1e6_And_RoundDownSafe 0 1234567 = 0

These examples demonstrate how the functions handle various scenarios, including
different scales, rounding behaviors, and edge cases. They illustrate the precision
and consistency of the scaling and rounding functions, particularly in mimicking
the behavior of PlutusTx.Ratio.truncate.
-}
